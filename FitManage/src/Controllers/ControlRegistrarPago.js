...
class ControlRegistrarPago {
    ...
    handleRegistrarPago = (req, res) => {
        ...
        PaquetesAdquiridos.getPaquetesVigentesByAlumno(req.body.id_alumno, 
            (err, rows) => {
                if (err) {
                    return res.status(500).json({ error: err.message });
                }
                    ...
                    Paquetes.getPaquetesById(req.body.id_paquete, 
                        (err, rows) => {
                            ...
                            if(rows.length > 0){
                                ...
                                PaquetesAdquiridos.registrarPaqueteAdquirido(datos, 
                                    (err, rows) => {
                                        if (err) {
                                            return res.status(500).json({ error: err.message });
                                        }
    ...
    handleObtenerAlumnos = (req, res) => {
        Alumno.getAlumnos((err, rows) => {
            ...
            // console.log(rows);
            if (rows.length > 0) {
                return res.render("PantallaSeleccionarEstudiantes",  { students: rows, error : ""})
            } else {
